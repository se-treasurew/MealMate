...
禁止提交 `.env`、PEM 私钥、SQLite 数据库、`backend/uploads/`、依赖目录和构建产物，仅提交 `.env.example`。认证密钥通过环境变量提供；Web Push 已关闭，历史 VAPID 私钥虽已停用，仍不得进入仓库。不要恢复外部推送调用或让通知失败影响订单事务。修改发布配置后必须重新运行发布树扫描。
